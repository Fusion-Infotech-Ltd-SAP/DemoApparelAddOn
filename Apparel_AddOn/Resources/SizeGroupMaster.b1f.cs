...
{
    [FormAttribute("Apparel_AddOn.Resources.SizeGroupMaster", "Resources/SizeGroupMaster.b1f")]
    class SizeGroupMaster : UserFormBase
    {
            ...
            try
            {
                ...
                if (oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OGSM"" WHERE ""Code"" = '{code.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OGSM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OGSM").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Size Group Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Size Group Name");
