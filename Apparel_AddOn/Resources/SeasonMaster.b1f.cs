...
{
    [FormAttribute("Apparel_AddOn.Resources.SeasonMaster", "Resources/SeasonMaster.b1f")]
    class SeasonMaster : UserFormBase
    {
        public SeasonMaster()
        {
        }
        ...
        private SAPbouiCOM.ComboBox CBACTIVE;
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OSEM"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OSEM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OSEM").GetValue("Name", 0);
            string active = oForm.DataSources.DBDataSources.Item("@FIL_MH_OSEM").GetValue("U_ACTIVE", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Season Master Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Season Master Name");
