...
{
    [FormAttribute("Apparel_AddOn.Resources.ProductCategory", "Resources/ProductCategory.b1f")]
    class ProductCategory : UserFormBase
    {
        public ProductCategory()
        {
        }
        private SAPbouiCOM.StaticText STCODE, STNAME, STACTIVE, STGROUP;
        private SAPbouiCOM.EditText ETCODE, ETNAME, ETDOCTRY;
        private SAPbouiCOM.Button ADDButton, CancelButton;
        private SAPbouiCOM.ComboBox CBACTIVE, CBGROUP;
        ...
        public override void OnInitializeComponent()
        {
            ...
            this.CBACTIVE = ((SAPbouiCOM.ComboBox)(this.GetItem("CBACTIVE").Specific));
            this.STGROUP = ((SAPbouiCOM.StaticText)(this.GetItem("STGROUP").Specific));
            this.CBGROUP = ((SAPbouiCOM.ComboBox)(this.GetItem("CBGROUP").Specific));
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OPCG"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPCG").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPCG").GetValue("Name", 0);
            string grp = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPCG").GetValue("U_PRDGRP", 0);
            string active = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPCG").GetValue("U_ACTIVE", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Product Category Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Product Category Name");
                oForm.ActiveItem = "ETNAME";
                return BubbleEvent = false;
            }
            else if (grp == "")
            {
                Global.GFunc.ShowError("Select Active Status");
                oForm.ActiveItem = "CBGROUP";
                return BubbleEvent = false;
            }
            else if (active == "")
            {
                Global.GFunc.ShowError("Select Active Status");
                oForm.ActiveItem = "CBACTIVE";
                return BubbleEvent = false;
            }
            return BubbleEvent;
        }
