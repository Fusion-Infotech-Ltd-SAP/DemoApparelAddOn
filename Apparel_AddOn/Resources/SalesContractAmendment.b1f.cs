...
{
    [FormAttribute("Apparel_AddOn.Resources.SalesContractAmendment", "Resources/SalesContractAmendment.b1f")]
    class SalesContractAmendment : UserFormBase
    {
        public SalesContractAmendment()
        {
        }
        private SAPbouiCOM.StaticText STCOMPNY, STCUSCOD, STBRNDCD, STSCNO, STSCDESC, STREFRNC, STVAL, STSCVAL, STDOCNUM, STBPBNK1,
                           STDOCDAT, STBPBNK2, STHSBNK, STCURR, STISUDAT, STSTATUS, STEXPDAT, STSHPDAT, STBUNIT, STB2B, STAMDNO, STAMMODE, STACMODE, STINTRMS, STRMSPAY, STDAYS, STMODSHP, STPTLDNG,
                           STCNDEST, STPDCHRG, STPTSHIP, STSHPTOL, STHSCODE, STDOCREQ, STRMSCON, STINRNCE, STNOTPTY, STREMARK, STMMODE, STCMODE;
        ...
        private SAPbouiCOM.EditText ETCUSCOD, ETCUSNAM, ETBRNDNM, ETBRNDCD, ETSCNO, ETSCVAL, ETDOCNUM, ETDOCDAT, ETBPBNK1, ETBPBNK2,
                    ...
                    ETEXPDAT, ETBUNIT, ETVAL, ETAMDNO, ETPTLDNG, ETPDCHRG, ETINRNCE, ETCNDEST, ETDAYS, ETPTSHIP, ETSHPTOL,
                    ETHSCODE, ETRMSCON, ETDOCREQ, ETNOTPTY, ETREMARK;

      

        private SAPbouiCOM.ComboBox CBCNTRY1, CBCNTRY2, CBBRNCH, CBCOMPNY, CBAMMODE, CBACMODE, CBINTRMS, CBRMSPAY, CBMODSHP, CBCMODE, CBMMODE;
        ...
        public override void OnInitializeComponent()
        {
            ...
            this.CBAMMODE.ComboSelectAfter += new SAPbouiCOM._IComboBoxEvents_ComboSelectAfterEventHandler(this.CBAMMODE_ComboSelectAfter);
            ...
            this.CBACMODE.LostFocusAfter += new SAPbouiCOM._IComboBoxEvents_LostFocusAfterEventHandler(this.CBACMODE_LostFocusAfter);
            this.CBACMODE.ComboSelectAfter += new SAPbouiCOM._IComboBoxEvents_ComboSelectAfterEventHandler(this.CBACMODE_ComboSelectAfter);
        ...
        private void Form_RightClickBefore(ref SAPbouiCOM.ContextMenuInfo eventInfo, out bool BubbleEvent)
        {
            ...
            if (ofrom.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE || ofrom.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
            {
      ...
      string deletedStyleCode)
        ...
        private void CBACMODE_LostFocusAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            SAPbouiCOM.Form oForm = Application.SBO_Application.Forms.Item(pVal.FormUID);
        ...
        private void CBACMODE_ComboSelectAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            SAPbouiCOM.Form oForm = Application.SBO_Application.Forms.Item(pVal.FormUID);
            if (pVal.ActionSuccess)
            {
                SAPbouiCOM.ComboBox combo = (SAPbouiCOM.ComboBox)oForm.Items.Item("CBACMODE").Specific;
        ...
        private bool ValidateForm(ref SAPbouiCOM.Form oForm, ref bool BubbleEvent)
         {
            ...
            if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
            {
                ...
                string sqlQuery = $"SELECT \"U_SCAMDNO\",\"U_CSCSTTS\",\"U_CSCASTTS\" FROM \"@FIL_DH_OSCM\" WHERE \"DocNum\" = {dnum}";
                oRec.DoQuery(sqlQuery);
                if (!oRec.EoF)
                {
                    mode = oRec.Fields.Item("U_CSCSTTS").Value.ToString();
                    amdNo = oRec.Fields.Item("U_SCAMDNO").Value.ToString();
                    amdmode = oRec.Fields.Item("U_CSCASTTS").Value.ToString();
                }
                int val = int.TryParse(amdNo, out var result) ? result : 0;
                if ((mode == "C" && val == 0) || (val != 0 && amdmode =="C") )
                {
                    Application.SBO_Application.MessageBox("Access Denied !!! Amend this Form ");
                    BubbleEvent = false;
                    return BubbleEvent;
                }
        ...
        private void CBAMMODE_ComboSelectAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            SAPbouiCOM.Form oForm = Application.SBO_Application.Forms.Item(pVal.FormUID);
            SAPbouiCOM.ComboBox oComboMerchant = (SAPbouiCOM.ComboBox)oForm.Items.Item("CBAMMODE").Specific;
            string selectedValue = oComboMerchant.Value;

            if (selectedValue == "C")
            {
                SAPbouiCOM.Item oItem = oForm.Items.Item("CBACMODE");
                oItem.Enabled = true;
            }
            else
            {
