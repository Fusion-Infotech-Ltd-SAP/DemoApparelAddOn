...
{
    [FormAttribute("Apparel_AddOn.Resources.SecondBankMaster", "Resources/SecondBankMaster.b1f")]
    class SecondBankMaster : UserFormBase
    {
        public SecondBankMaster()
        {
        }
        private SAPbouiCOM.StaticText STCODE, STNAME, STADDRES;
        private SAPbouiCOM.EditText ETCODE, ETNAME, ETDOCTRY, ETADDRES;
        ...
        public override void OnInitializeComponent()
        {
            ...
            this.STADDRES = ((SAPbouiCOM.StaticText)(this.GetItem("STADDRES").Specific));
            this.ETADDRES = ((SAPbouiCOM.EditText)(this.GetItem("ETADDRES").Specific));
            ...
            try
            {
                ...
                if (oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_SCNBANK"" WHERE ""Code"" = '{code.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_SCNBANK").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_SCNBANK").GetValue("Name", 0);
            string address = oForm.DataSources.DBDataSources.Item("@FIL_MH_SCNBANK").GetValue("U_ADDRESS", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Secondary Bank Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Secondary Bank Name");
                oForm.ActiveItem = "ETNAME";
                return BubbleEvent = false;
            }
            else if (address == "")
            {
                Global.GFunc.ShowError("Enter Secondary Bank Address");
                oForm.ActiveItem = "ETADDRES";
                return BubbleEvent = false;
            }
            return BubbleEvent;
        }
