...
{
    [FormAttribute("Apparel_AddOn.Resources.SleeveTypeMaster", "Resources/SleeveTypeMaster.b1f")]
    class SleeveTypeMaster : UserFormBase
    {
        public SleeveTypeMaster()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OSVM"" WHERE ""Code"" = '{code.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OSVM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OSVM").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Sleeve Type Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Sleeve Type Name");
