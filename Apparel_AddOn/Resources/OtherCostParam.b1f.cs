...
{
    [FormAttribute("Apparel_AddOn.Resources.OtherCostParam", "Resources/OtherCostParam.b1f")]
    class OtherCostParam : UserFormBase
    {
        public OtherCostParam()
        {
        }
        private SAPbouiCOM.StaticText STCODE, STNAME, STDESC, STACTIVE;
        private SAPbouiCOM.EditText ETCODE, ETNAME, ETDOCTRY, ETDESC;
            ...
            try
            {
                ...
                if (oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OTHCPMST"" WHERE ""Code"" = '{code.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OTHCPMST").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OTHCPMST").GetValue("Name", 0);
            string desc = oForm.DataSources.DBDataSources.Item("@FIL_MH_OTHCPMST").GetValue("U_DESC", 0);
            if (code == "")
            {
            ...
            {
                Global.GFunc.ShowError("Enter Cost Description");
