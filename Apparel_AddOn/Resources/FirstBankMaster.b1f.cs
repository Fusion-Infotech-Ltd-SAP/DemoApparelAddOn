...
{
    [FormAttribute("Apparel_AddOn.Resources.FirstBankMaster", "Resources/FirstBankMaster.b1f")]
    class FirstBankMaster : UserFormBase
    {
        public FirstBankMaster()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OFBM"" WHERE ""Code"" = '{code.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OFBM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OFBM").GetValue("Name", 0);
            string address = oForm.DataSources.DBDataSources.Item("@FIL_MH_OFBM").GetValue("U_ADDRESS", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter First Bank Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter First Bank Name");
                oForm.ActiveItem = "ETNAME";
                return BubbleEvent = false;
            }
            else if (address == "")
            {
                Global.GFunc.ShowError("Enter First Bank Address");
