...
{
    [FormAttribute("Apparel_AddOn.Resources.SizeParamMaster", "Resources/SizeParamMaster.b1f")]
    class SizeParamMaster : UserFormBase
    {
        public SizeParamMaster()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_SIZEPARAM"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_SIZEPARAM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_SIZEPARAM").GetValue("Name", 0);
