...
{
    [FormAttribute("Apparel_AddOn.Resources.StyleDepartment", "Resources/StyleDepartment.b1f")]
    class StyleDepartment : UserFormBase
    {
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_STLDEPT"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
                        if (!oRS.EoF)
                        {
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_STLDEPT").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_STLDEPT").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Style Deparment Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Style Department Name");
