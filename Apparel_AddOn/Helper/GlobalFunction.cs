...
namespace Apparel_AddOn.Helper
{
    class GlobalFunction
    {
        ...
        public void AddRPTUDO()
        {
            string rptdateudo;
            SAPbobsCOM.Recordset ors;
            ors = (SAPbobsCOM.Recordset)Global.oComp.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            string str = "Select \"CODE\" FROM \"RTYP\" WHERE \"MNU_ID\"='FIL_FRM_COSTING'";
            ors.DoQuery(str);
            // authorization , report, approval , DIAPi integration all will b considered as t service
            SAPbobsCOM.CompanyService ocmpser;
            SAPbobsCOM.ReportTypesService rpttypeservice;
            ocmpser = (SAPbobsCOM.CompanyService)Global.oComp.GetCompanyService();
            rpttypeservice = (SAPbobsCOM.ReportTypesService)ocmpser.GetBusinessService(SAPbobsCOM.ServiceTypes.ReportTypesService);
            ...
            newtype.TypeName = "FIL_FRM_COSTING";
            newtype.AddonName = "Apparel_AddOn";
            newtype.AddonFormType = "FIL_FRM_COSTING";
            newtype.MenuID = "FIL_FRM_COSTING";
            ...
            if (ors.RecordCount == 0)
            {
                ...
                ors1 = (SAPbobsCOM.Recordset)Global.oComp.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                string strocrg = "Select \"DocCode\" FROM \"RDOC\" WHERE \"DocName\"='FIL_FRM_COSTING'";
                ors1.DoQuery(strocrg);

                if (ors1.RecordCount == 0)
                {
                    SAPbobsCOM.ReportLayoutsService rptservice;
                    rptservice = (SAPbobsCOM.ReportLayoutsService)ocmpser.GetBusinessService(SAPbobsCOM.ServiceTypes.ReportLayoutsService);
                    ...
                    newreport.Author = Global.oComp.UserName;
                    newreport.Category = SAPbobsCOM.ReportLayoutCategoryEnum.rlcCrystal;
                    newreport.Name = "FIL_FRM_COSTING";
                    ...
                    oblobparams = (SAPbobsCOM.BlobParams)Global.oComp.GetCompanyService().GetDataInterface(SAPbobsCOM.CompanyServiceDataInterfaces.csdiBlobParams);
                    oblobparams.Table = "RDOC";
                    oblobparams.Field = "Template";
                    SAPbobsCOM.BlobTableKeySegment okeysegement;
                    okeysegement = oblobparams.BlobTableKeySegments.Add();
                    okeysegement.Name = "DocCode";
                    okeysegement.Value = newreportparam.LayoutCode;
                    rptdateudo = newreportparam.LayoutCode;
                }
                else
                {
                    rptdateudo = ors1.Fields.Item("DocCode").Value.ToString();
                }
